...
export function element_fields(i) {
  ...
  r.hits = 10;
  return [before.hits, $index(cells, i).hits, $index(cells, (1 - i) >>> 0).hits];
}
...
export function set_basics(n) {
  ...
  return [
    ...
    "{" +
      Array.from(one)
        .map(([key, value]) => $debugStr(key) + ": " + String(value))
        .join(", ") +
      "}",
  ];
}
...
export function sorted_maps(text) {
  ...
  const tmp$2 =
    "{" +
    Array.from($sortedKeys(set, $cmp))
      .map((item) => String(item))
      .join(", ") +
    "} [" +
    arg.map((item) => String(item)).join(", ") +
    "]";
  ...
  return [
    ...
    tmp$2,
    "{" +
      Array.from($sortedEntries(tiers, (a, b) => $cmpIn(["Gold", "Bronze", "Silver"], a, b)))
