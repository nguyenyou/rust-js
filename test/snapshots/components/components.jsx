...
import { createContext, memo, useContext, useEffect, useId, useMemo, useReducer, useRef, useState } from "react";
...
export function Todos() {
  ...
  const items = todos.map((t) => {
    ...
    return <li key={t.id} className={t.done ? "done" : ""} onClick={onClick}>{t.text}</li>;
  });
  return <Card title="Todos">
    <>
...
export function Clock() {
  ...
  return <>
    ...
    <button className="tick" onClick={() => setTicks((t) => t + 1 | 0)}>tick</button>
  </>;
}
...
export function Themed() {
  ...
  return <>
    ...
    <button className="theme" onClick={() => setDark((d) => !d)}>theme</button>
    <button className="click" onClick={() => setClicks((c) => c + 1 | 0)}>{clicks}</button>
  </>;
}
