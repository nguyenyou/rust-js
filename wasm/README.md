...
the stats table, the file trees, `compile`, which runs `rust-js.wasm` under the WASI shim, and running the program in the Result frame). `serve.ts` and `build.ts` compile it to `web/rust/lib.js`
